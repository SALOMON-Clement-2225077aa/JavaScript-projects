...
const EL_temperatureTemplate = document.getElementById('temperature-template').content;
...
class Logger1 {
    ...
    update(data) {
        if(data.Valeur != this.I_currentTemperature) {
            ...
            const clone = document.importNode(EL_temperatureTemplate, true);
            clone.querySelector('#temperature-value').textContent = this.I_currentTemperature + '°C';
            clone.querySelector('#temperature-timestamp').textContent = 'Date et heure : ' + new Date(data.Timestamp * 1000).toLocaleString();
            EL_currentTemperatureElement.innerHTML = '';
            EL_currentTemperatureElement.appendChild(clone);
